...
- [x] Explain the difference between `Relational Databases` and `SQL`.
  ...
  Relational Databases are where you have data split up into different tables that relate to each other through foreign keys. SQL is the structured query language that we use to CRUD our database.

- [x] Why do tables need a `primary key`?

  Tables need a primary key because each entry needs a unique identifier, this is how we reference it in a different table as a foreign key.

- [x] What is the name given to a table column that references the primary key on another table.

  a foreign key, it is used to check for conditions to determine what to do with the SQL code.

- [x] What do we need in order to have a _many to many_ relationship between two tables.

  we need to use a separate table that references (through foreign keys) the keys on the other tables.
